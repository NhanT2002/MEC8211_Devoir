...
def graphique_convergence_espace(prm, sol_anal, mdf_oi):
    '''
    ...
    '''
    # Mailles differentes
    N_range = np.logspace(0.5, 3.0, 10, dtype=int)
    ...
    for n in N_range:
        ...
        C_anal = sol_anal(prm)
        r, dr, C_mdf_oi = mdf_oi(prm, source_MMS)
        dr_range.append(dr)
    ...
    order_L1, intercept_L1 = np.polyfit(np.log(dr_range[-5:-1]), np.log(L1_range[-5:-1]), 1)
    order_L2, intercept_L2 = np.polyfit(np.log(dr_range[-5:-1]), np.log(L2_range[-5:-1]), 1)
    order_Linf, intercept_Linf = np.polyfit(np.log(dr_range[-5:-1]), np.log(L_inf_range[-5:-1]), 1)
    ...
    plt.title(f"Convergence de l'erreur en fonction de $\Delta r$ pour $\Delta t$ = {prm.T/(prm.K-1):.2e} s")
    plt.xscale('log')
    plt.yscale('log')
    plt.grid()
...
def graphique_convergence_temps(prm, sol_anal, mdf_oi):
    '''
    Fonction qui trace le graphique de l'ordre de covergence de l'erreur 
    observée selon la norme L_1, L_2 et L_infinie pour la résolution d'une 
    solution par la  méthode des différences finies (MDF), 
    ...
    '''
    # Mailles differentes
    K_range = np.logspace(0.5, 4.0, 10, dtype=int)
    #Initialisation pour le schema d'ordre spécifié
    L1_range = []
    L2_range = []
    L_inf_range = []
    ...
    # Ajouter des étiquettes et un titre au graphique
    plt.legend()
    plt.xlabel(f'$\Delta t$ [m]')
    plt.ylabel('Erreur [mol/$m^3$]')
    plt.title(f"Convergence de l'erreur en fonction de $\Delta t$ pour $\Delta r$ = {prm.R/(prm.N-1)} s")
    plt.xscale('log')
    plt.yscale('log')
    plt.grid()
