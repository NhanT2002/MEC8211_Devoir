...
class parametres():
    ...
    T = 4e9          # Temps final [s]
    K = 10000         # Nombre de pas de temps [-]
    C_i = C_e          # Concentration initiale [mol/m^3]

prm = parametres()

# # Résolution du problème
# C_anal_MMS = concentration_anal_MMS(prm) # solution analytique
# r, dr, C_mdf_o2 = concentration_mdf_o2(prm, source_MMS) # solution numérique, schémas d'ordre 2

# # Plot des concentrations : solutions analytique et numérique
# fig = plt.figure()
# ax = fig.add_subplot()
# for i in range(0, prm.K, 500):
#     ax.plot(r, C_mdf_o2[i], label=f'Solution numérique, t = {i}')
#     ax.plot(r, C_anal_MMS[i], '--', lw = 1, label=f'Solution analytique, t = {i}')
...
graphique_convergence_espace(prm, concentration_anal_MMS, concentration_mdf_o2) #ordre 1
# graphique_convergence_temps(prm, concentration_anal_MMS, concentration_mdf_o2) #ordre 1
