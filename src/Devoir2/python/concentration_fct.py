...
def concentration_anal_MMS(prm):
    ...
    r, t = np.meshgrid(r, t)
    return prm.C_e*(np.exp(1.0e-10*t) - 1)*np.cos(np.pi*r**2/(2*prm.R**2)) + prm.C_e
...
def source_MMS(prm, r, t):
    ...
    sin = np.sin
    pi = np.pi
    f = C_e*(-pi*Deff*(1 - exp(1.0e-10*t))*(2*R**2*sin(pi*r**2/(2*R**2)) \
    + pi*r**2*cos(pi*r**2/(2*R**2))) + R**4*(k*(-(1 - exp(1.0e-10*t))*cos(pi*r**2/(2*R**2)) + 1) \
    + 1.0e-10*exp(1.0e-10*t)*cos(pi*r**2/(2*R**2))))/R**4
    return f
